...
const SupplierPickerScreen = () => {
  ...
  const load = useCallback(async query => {
    ...
    setError(null);
    try {
      const data = await getSuppliers({page: 1, limit: 20, search: query || ''});
  ...
  return (
    <View style={styles.flex}>
      <SearchPickerList
        ...
        error={error}
        onRetry={() => load(search)}
        addLabel={t('addSupplierAction')}
        onAddPress={() => navigation.navigate('AddSupplier', {onCreated: handleSelect})}
      />
    </View>
  );
};
